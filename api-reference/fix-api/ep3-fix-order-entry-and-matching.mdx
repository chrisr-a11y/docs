---
title: "FIX Order Entry & Matching"
---
...
The PMX platform offers self-match prevention logic which can be enabled either at the FIX session level (automatically applied to all orders entered through a session), or on a per-order basis using the SelfMatchPreventionID (7928) and SelfMatchPreventionInstruction (8000) tags.
...
Each instrument on the PMX Platform has a minimum price increment (“tick size”) and minimum quantity increment (“lot size”). These instrument-level settings are indicated in the SecurityList \[y] message in the MinPriceIncrement (969) and MinTradeVol (562) fields respectively.
...
The intended behavior of orders entering the PMX Platform is controlled using a combination of OrderType (40), TimeInForce (59) and ExecInst (18) fields. These are explained in turn below.
...
| SINGLE EXECUTION REQUESTED FOR BLOCK TRADE | j     | A flag for submitting block trades to PMX.                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
Participants may place orders into the PMX platform order book to buy or sell securities using a NewOrderSingle \[D] message.
...
| 17               | ExecID                         | Y     | String    | Unique identifier for the Execution Report as assigned by PMX. Typically a 13-character alphanumeric string.                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 37               | OrderID                        | Y     | String    | Unique identifier for Order as assigned by PMX. Typically a 13-character alphanumeric string.                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| 150              | ExecType                       | Y     | char      | The reason that the PMX Platform sent this Execution Report.\<br>0 = New\<br>4 = Canceled\<br>5 = Replaced\<br>8 = Rejected\<br>C = Expired\<br>F = Trade                                                                                                                                                                                                                                                                                                                                             |
...
In this case, the order to be amended is identified using the last-entered ClOrdID (11) value relating to the order into the OrigClOrdID (41) field. Following FIX convention, this modification message will also contain a fresh ClOrdID (11) value which can then be used by the Participant to further modify the order if required. Note that the PMX Platform does not require the entry of the (platform-generated) OrderID (37) in order to amend the order.

### Table 17: OrderCancelReplaceRequest (G) message

If the order has been successfully modified, the PMX Platform will respond with an ExecutionReport \[8] message which echoes many of the key (updated) order attributes, and with ExecType (150) = 5 (replaced).
...
If the order can not be modified for any reason (for example if the requested price / order size are not acceptable, or the order has already expired), then the PMX Platform will respond with an OrderCancelReject \[9] message indicating the reason.
...
If the order has been canceled, the PMX Platform will respond with an ExecutionReport \[8] message which echoes many of the key (updated) order attributes, and with ExecType (150) = 4 (canceled).
...
If the request to cancel the order passed validation, then the PMX Platform will acknowledge the order cancellation with an ExecutionReport \[8] message indicating OrdStatus (39) = 4 (canceled), and LeavesQty (151) = 0.
