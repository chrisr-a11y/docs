...
The Logon \[A] message initiates a connection to the PMX platform. FIX sessions are typically initiated by customers (as opposed to the exchange platform), and can be made any time.
...
In the event that one of these messages can not be used to reject a message, however, then the PMX Platform may return either a Reject \[3] message, or a BusinessMessageReject \[j] message as described below.\
