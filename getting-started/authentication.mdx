...
description: 'Getting started with API authentication for ISVs, IBs, and FCMs'
---
...
The Polymarket Exchange API uses **Private Key JWT** authentication. This is a more secure method than client secrets - you generate your own key pair and keep the private key secure.
...
3. **Call API with access token** - The API validates your token and handles authorization
...
| Company Information | Auth Domain |
...
Share your public key with us securely:

<Steps>
  <Step title="Create a Google Doc">
    Create a new Google Document titled "**[Your Company Name] - Polymarket API Public Key**"
  </Step>
  <Step title="Copy your public key">
    Copy the entire contents of `my_public_key.pem` into the document:
    ```
    -----BEGIN PUBLIC KEY-----
  ...
  <Step title="Add your details">
    ...
    - IP addresses for API access (static IPs required)
    - Environment requested (dev, preprod, production)
  </Step>
  <Step title="Share with Polymarket">
    Share the Google Doc with **fixapi@polymarket.com** (Editor access)
  </Step>
</Steps>
...
The public key file starts with `-----BEGIN PUBLIC KEY-----`. Never share your private key (`-----BEGIN RSA PRIVATE KEY-----` or `-----BEGIN PRIVATE KEY-----`) with anyone, including Polymarket support.
</Warning>
...
| Environment | Auth Domain | API Base URL |
|-------------|-------------|--------------|
| Development | `auth.dev.polymarketexchange.com` | `api.dev01.polymarketexchange.com` |
| Pre-production | `auth.preprod.polymarketexchange.com` | `api.preprod.polymarketexchange.com` |
| Production | `auth.polymarketexchange.com` | `api.prod.polymarketexchange.com` |
