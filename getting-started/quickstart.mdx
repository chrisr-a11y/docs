...
description: "Place your first order in 5 minutes"
---
...
<Info>
**Prerequisites:** You need API credentials from onboarding. See [Getting Access](/getting-started/getting-access) if you don't have them yet.
</Info>
...
# Extract public key (send this to us during onboarding)
openssl rsa -in private_key.pem -pubout -out public_key.pem
```

<Warning>
Never share your private key. Keep it secure with restricted file permissions (`chmod 600 private_key.pem`).
</Warning>
...
AUTH_DOMAIN = "auth.preprod.polymarketexchange.com"
...
# Check who you are
response = requests.post(f"{BASE_URL}/v1/accounts/whoami", headers=headers, json={})
response.raise_for_status()
...
## Step 4: List Available Instruments
...
symbol = "mlb-ari-sf-2025-09-08"  # Replace with actual symbol
price_scale = 10000  # Get this from instrument metadata
...
## Step 8: Cancel Your Order (Optional)
...
# Cancel by order ID
response = requests.post(
    f"{BASE_URL}/v1/trading/order/cancel",
    headers=headers,
    json={
...
def main():
    ...
    # 2. Verify identity
    print("\n2. Checking identity...")
    resp = requests.post(f"{BASE_URL}/v1/accounts/whoami", headers=headers, json={})
    ...
    price_scale = instruments[0].get("price_scale", 10000)
    ...
    resp = requests.post(f"{BASE_URL}/v1/trading/order", headers=headers, json=order)
...
<CardGroup cols={2}>
  <Card title="Authentication Deep Dive" icon="key" href="/getting-started/auth0-onboarding">
