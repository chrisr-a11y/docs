...
1. Contact support to obtain the `ep3-py-client` and its associated files
...
| Field | Type | Description |
|-------|------|-------------|
| `px` | `int64` | **Price** (divide by 1000 for decimal value) |
...
if update.HasField('stats'):
    ...
    if stats.HasField('last_trade_px'):
        last_px = stats.last_trade_px / 1000
        print(f"Last Trade: ${last_px:.4f}")
```

| Field | Type | Description |
|-------|------|-------------|
| `open_px` | `int64` | Opening price (÷ 1000) |
| `close_px` | `int64` | Closing price (÷ 1000) |
| `low_px` | `int64` | Session low price (÷ 1000) |
| `high_px` | `int64` | Session high price (÷ 1000) |
| `last_trade_px` | `int64` | Last trade price (÷ 1000) |
...
| `avg_px` | `int64` | **Average fill price** (÷ 1000) |
...
if execution.last_shares > 0:
    fill_px = execution.last_px / 1000
...
```python
# Proto → Decimal
decimal_price = int64_price / 1000

# Example
proto_price = 525
decimal_price = 525 / 1000  # = $0.525
```
</Warning>
...
if update.HasField('stats'):
    if update.stats.HasField('last_trade_px'):
        last_px = update.stats.last_trade_px / 1000
```
