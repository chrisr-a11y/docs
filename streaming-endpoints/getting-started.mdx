...
- **Auth0 Credentials**: Client ID and Client Secret from [onboarding](/getting-started/auth0-onboarding)
...
# Describe a service
grpcurl traderapi.us-east-1.privatelink.preprod.polymarketexchange.com:443 describe polymarket.v1.MarketDataSubscriptionAPI
```
...
python -m grpc_tools.protoc \
  ...
  --grpc_python_out=. \
  --proto_path=protos \
  protos/polymarket/v1/market_data.proto \
  protos/polymarket/v1/order_entry.proto \
  protos/polymarket/v1/orders.proto \
  protos/polymarket/v1/instruments.proto \
  protos/polymarket/v1/common.proto
```
...
## Step 4: Authenticate via Auth0

<Warning>
**CRITICAL: Tokens must be refreshed every 3 minutes.**

Auth0 tokens have a short expiration. Your application MUST implement automatic token refresh before expiration to maintain uninterrupted streaming connections.
</Warning>
...
Obtain a JWT token using the OAuth 2.0 Client Credentials flow:
...
<Info>
See the [Auth0 Onboarding Guide](/getting-started/auth0-onboarding) for complete authentication details.
</Info>
...
from polymarket.v1 import market_data_pb2
from polymarket.v1 import market_data_pb2_grpc
...
# Set up metadata with authorization (JWT from Auth0)
metadata = [
...
- **Solution**: Verify JWT from Auth0, refresh token if expired (tokens expire every 3 minutes)
