...
| Field | Type | Description |
|-------|------|-------------|
| `px` | `int64` | **Price as integer** (divide by 1000 for decimal) |
...
**Example:**
- Raw value: `500`
- Decimal price: `500 / 1000 = 0.5 USD`
- Display as: `$0.50`
...
| `high_px` | `int64` | High price of session (÷ 1000) |
| `low_px` | `int64` | Low price of session (÷ 1000) |
        ...
        if response.HasField('heartbeat'):
            ...
            if update.offers:
                ...
                for i, offer in enumerate(update.offers[:5]):  # Show top 5 offers
                    px = offer.px / 1000  # Convert from price representation
                    qty = offer.qty
                    print(f"    [{i+1}] ${px:.4f} x {qty}")
