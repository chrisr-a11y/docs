...
```python
from polymarket.v1 import order_entry_pb2
...
| `price` | `int` | **Order price** (÷ price_scale for decimal) |
| `cum_qty` | `int` | Cumulative filled quantity |
| `leaves_qty` | `int` | Remaining unfilled quantity |
| `avg_px` | `int` | **Average fill price** (÷ price_scale) |
...
```python
from polymarket.v1 import orders_pb2
...
| `last_px` | `int` | **Price of this fill** (÷ price_scale) |
...
from polymarket.v1 import order_entry_pb2
from polymarket.v1 import order_entry_pb2_grpc
from polymarket.v1 import orders_pb2
...
class PolymarketOrderStreamer:
    ...
    def login(self, auth0_domain: str, client_id: str, client_secret: str, audience: str) -> dict:
        """Authenticate with Auth0 and store the access token."""
        ...
        self.access_expiration = datetime.now() + timedelta(seconds=expires_in - 300)
