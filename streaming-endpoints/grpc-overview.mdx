...
1. **REST API** - Used for:
   - Placing orders (`POST /v1/trading/orders`)
   - Canceling orders (`DELETE /v1/trading/orders/{order_id}`)
   - Querying account information (`GET /v1/accounts/whoami`)
   - Searching orders and trades (`POST /v1/orders/search`)
...
- **Upon request**: Contact [support@qcex.com](mailto:support@qcex.com)
...
polymarket.v1    # Order message types
polymarket.v1  # Instrument definitions
```
...
```mermaid
graph LR
    A[Python Client] -->|Client Credentials| B[Auth0]
    A -->|REST| C[REST API]
    A -->|gRPC Stream| D[gRPC Server]
    B -->|JWT Token| A
...
1. **Obtain JWT Token**: Request token from Auth0 using client credentials
...
4. **Refresh Token**: Tokens expire every 3 minutes - implement automatic refresh
